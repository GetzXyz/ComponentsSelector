  ...
  try {
    const response = await fetch("https://api.groq.com/openai/v1/chat/completions", {
      ...
      body: JSON.stringify({
        ...
        temperature: 0.2,
        messages: [
          { role: "system", content: "You are FORGE AI, a PC hardware expert. Return only valid JSON matching the provided shape. Use realistic current market estimates, regional pricing, compatibility reasoning, benchmark-driven FPS estimates, and never guarantee FPS." },
          { role: "user", content: JSON.stringify({ task: "Create a complete PC build with CPU, GPU, motherboard, RAM, SSD, PSU, case, cooler, monitor, market notes, FPS estimates for 10 named games, peripherals, and high-end optimization notes.", input: parsed.data, requiredShape: fallback }) },
        ],
      }),
    });
    if (!response.ok) throw new Error(await response.text());
    const data = await response.json();
    const ai = JSON.parse(data.choices?.[0]?.message?.content) as BuildResult;
